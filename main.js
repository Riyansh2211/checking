...
classifier= ml5.imageClassifier('https://teachablemachine.withgoogle.com/models/ezdEfOvZd/model.json',modelLoaded);
...
function gotResult(error, results){
    if(error){
        ...
        if(results[0].label=='improper'){
            document.getElementById("update_emoji").innerHTML= "Imprpoer mask";
        }

        if(result[0].label=='no'){
            document.getElementById("update_emoji").innerHTNL= "Proper mask";
        }


        if(results[1].label=='proper'){
            document.getElementById("update_emoji").innerHTML= "No mask";

        }

        if(results[1].label=='improper'){
            document.getElementById("update_emoji").innerHTML= "Imprpoer mask";
        }
